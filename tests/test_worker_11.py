...
def test_map_memory(mocker, tmp_path, monkeypatch):
    ...
    Note: 'monkeypatch' is a fixture provided by the pytest package.
    This fixture lets us temporarily modify environment variables, object
    attributes, or dictionary values during a test (for example, to override
    environment flags or configuration settings).

    See https://docs.pytest.org/en/6.2.x/monkeypatch.html for more info.
    """
    ...
    # Override locale setting to 'C' to standardize UNIX sort across platforms
    ...
    # We expect one subprocess.Popen() call per output file, plus two for
    # sorting.
    assert count_popen_calls.call_count == 6

    # Verify map task output
    assert_dirs_eq(tmp_path, TESTDATA_DIR/"test_worker_11")
